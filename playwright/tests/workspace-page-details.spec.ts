...
test.describe('Workspace Page Details', () => {
  ...
  test.describe('Action Toolbar', () => {
    ...
    test('create button aria-label matches locale', async () => {
      expect(await harness.createButton.getAttribute('aria-label')).toBe(t('create_button_aria'))
    })
  })
  ...
  test.describe('DataView and Screenshot', () => {
    test('dataview section is visible', async () => {
      await expect(harness.dataView).toBeVisible()
    })
    ...
    test('saves full-page screenshot', async () => {
      await harness.page.waitForLoadState('networkidle')
      await harness.page.screenshot({
        path: `${outputDir}/screenshots/workspace-details.png`,
        fullPage: true,
      })
    })
  })
